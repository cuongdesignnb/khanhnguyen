...
export async function POST(request: NextRequest) {
  ...
  const job = await prisma.aiGenerationJob.create({
    data: {
      ...
      textModel: String(body.textModel || setting?.textModel || process.env.OPENAI_MODEL || 'gpt-5.5'),
      imageModel: String(body.imageModel || setting?.imageModel || process.env.OPENAI_IMAGE_MODEL || 'gpt-image-2'),
